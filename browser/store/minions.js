...
export const createMinionThunk = minion => dispatch => {
  axios.post(`${process.env.API_BASE_URL}/api/minions`, minion)
  .then(res => res.data)
  .then(createdMinion => {
...
export const updateMinionThunk = minion => dispatch => {
  axios.put(`${process.env.API_BASE_URL}/api/minions/${minion.id}`, minion)
  .then(res => res.data)
  .then(updatedMinion => {
...
export const deleteMinionThunk = minionId => dispatch => {
  axios.delete(`${process.env.API_BASE_URL}/api/minions/${minionId}`)
  .then(res => res.data)
  .then(() => {
    return axios.get(`${process.env.API_BASE_URL}/api/minions`)
  })
  .then(res => res.data)
  .then(allMinions => {

...
const appEnter = nextRouterState => {
  Promise.all([
    axios.get(`${process.env.API_BASE_URL}/api/minions`),
    axios.get(`${process.env.API_BASE_URL}/api/ideas`),
    axios.get(`${process.env.API_BASE_URL}/api/meetings`),
  ])
...
const singleMinionEnter = nextRouterState => {
  ...
  axios.get(`${process.env.API_BASE_URL}/api/minions/${id}`)
  .then(res => res.data)
  .then(minion => {
  ...
  axios.get(`${process.env.API_BASE_URL}/api/minions/${id}/work`)
  .then(res => res.data)
  .then(work => {
...
const singleIdeaEnter = nextRouterState => {
  const id = nextRouterState.params.id;
  axios.get(`${process.env.API_BASE_URL}/api/ideas/${id}`)
  .then(res => res.data)
  .then(idea => {
